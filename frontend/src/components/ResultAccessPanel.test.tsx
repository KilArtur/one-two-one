...
vi.mock("../api/client");
const link: api.ResultLinkInfo = {
  id: "link",
  candidate_id: "candidate",
  created_by: "recruiter",
  expires_at: "2099-10-05T00:00:00Z",
  revoked_at: null,
};
afterEach(() => {
  cleanup();
...
beforeEach(() => {
  ...
  vi.mocked(api.getVideoViews).mockResolvedValue([
    {
      id: "view",
      answer_id: "answer",
      viewer: "expert",
...
it("resolves the shared result and passes the secret to scoped card requests", async () => {
  ...
  render(
    <MemoryRouter>
      <ResultLinkPage token="jwt" />
    </MemoryRouter>,
  );
  await screen.findByText("К-candidate");
