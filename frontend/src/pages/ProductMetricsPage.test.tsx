...
function renderMetrics(vacancyId?: string) {
  return render(
    <MemoryRouter>
      <ProductMetricsPage token="token" vacancyId={vacancyId} />
    </MemoryRouter>,
  );
}
...
function data(): ProductMetrics {
  ...
  return {
    ...
    review_directions: [],
    completion: { count: 1, total: 1, share: 1 },
...
it("shows fractions and refreshes review and completion after changes", async () => {
  ...
  const updated = {
    ...
    completion: { count: 1, total: 2, share: 0.5 },
    technical_failures: { count: 1, total: 2, share: 0.5 },
  };
  vi.mocked(getProductMetrics).mockResolvedValueOnce(initial).mockResolvedValueOnce(updated);
  renderMetrics("vacancy");
