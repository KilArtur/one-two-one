...
def render_resume_text(draft: ResumeDraft) -> str:
    """Собирает карточку в текст резюме, по которому персонализируются вопросы."""
    blocks: list[str] = []
    if draft.headline:
...
async def build_resume_card(
    ...
    """Разбирает текст резюме в карточку кандидата; ничего не сохраняет."""
    llm_client = llm_client or get_llm_client()
    prompt = load_prompt(RESUME_DRAFT_PROMPT).format(document_text=document_text)
    ...
    return ResumeCard(profile=draft, resume_text=render_resume_text(draft))
